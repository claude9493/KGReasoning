...
class KGReasoning(nn.Module):
    ...
    def forward_beta(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        ...
        if type(negative_sample) != type(None):
            ...
            negative_logit = None
        # p = (model.gamma - all_negative_logit) / sum((model.gamma - all_negative_logit))
        p = (negative_logit) / sum(negative_logit)  # Lower distance, higher weights
        negative_logit = np.random.choice(negative_logit, size=len(negative_logit), replace=True, p=p)
